...
use web_sys::{
    window, CanvasRenderingContext2d, HtmlCanvasElement, ImageData, Performance, PermissionState,
    PermissionStatus,
};
...
impl ClipboardWorker {
    ...
    fn get_canvas_and_context() -> Result<(HtmlCanvasElement, CanvasRenderingContext2d), String> {
        let canvas = window()
            .ok_or("Window not found".to_owned())?
            .document()
            .ok_or("Document not found in window".to_owned())?
            .get_element_by_id("canvas")
            .ok_or("canvas not found in document")?
            .dyn_into::<HtmlCanvasElement>()
            .map_err(|err| format!("Failed to cast to HtmlCanvasElement: {:?}", err))?;
        //.expect("Failed to cast to HtmlCanvasElement");

        let tmp1: Object = canvas
            .get_context("2d")
            .map_or_else(
    ...
    async fn process_cb_query(query_promise: Promise) -> Result<(), String> {
        let query_res = JsFuture::from(query_promise).await.map_err(|err| {
            format!(
                "Failure awaiting copy permission query result: error {:?}",
                err
            )
        })?;
        ...
        if status.state() == PermissionState::Granted {
            ...
            Err(format!(
                "copy_to_clipboard: not permitted to copy to clipboard: {:?}",
                status.state()
            ))
        }
    }
    ...
    fn handle_init(&mut self, id: HandlerId) -> Result<StageRes, String> {
        ...
        Reflect::set(
            &query_obj,
            &JsValue::from("name"),
            &JsValue::from("clipboard-write"),
        )
        .map_err(|err| format!("Failed to write query to object, error: {:?}", err))?;

        // info!("copy_to_clipboard: got query_object: {:?}", query_obj);
        let query_res = window()
            .ok_or("Window not found".to_owned())?
            .navigator()
            .permissions()
            .map_err(|err| format!("Permissions not found, error: {:?}", err))?
            .query(&query_obj)
            .map_err(|err| format!("Failed to query permissions, error: {:?}", err))?;
    ...
    fn handle_flip(&mut self, stage_data: RawData) -> Result<StageRes, String> {
        ...
        for row in (0..stage_data.rows).rev() {
            flipped_u8.extend_from_slice(&data[(row * bytes_per_row)..((row + 1) * bytes_per_row)]);
        }
        assert_eq!(data.len(), flipped_u8.len());
        info!(
            "handle_flip bytes_per_row: {}, rows: {}, data.len(): {}, flipped len: {}",
            bytes_per_row,
            stage_data.rows,
            data.len(),
            flipped_u8.len()
    ...
    fn handle_png_encode(&mut self, data: FlippedData) -> Result<StageRes, String> {
        ...
        self.stage = Stage::MakeItems(PngData { data: img_u8 });
        Ok(StageRes::Done)
    }
    ...
    async fn process_copy(data: js_sys::Array) -> Result<(), String> {
        let clipboard = window()
            ...
            .navigator()
            .clipboard()
            .ok_or("Clipboard not found".to_owned())?;

        JsFuture::from(clipboard.write(&data.into()))
            .await
...
impl Agent for ClipboardWorker {
    ...
    fn handle_input(&mut self, _msg: Self::Input, id: HandlerId) {
        ...
        info!(
            "ClipboardWorker::handle_input: stage: {:?} @ offset {} secs",
            self.stage, offset_sec
        );
